# Закодируйте любую строку по алгоритму Хаффмана.

from collections import Counter
from operator import itemgetter


class MyNode:
    def __init__(self, left=None, right=None):
        self.left = left
        self.right = right

    def walk(self, bin_, value):
        self.left.walk(bin_, value + ['0'])
        self.right.walk(bin_, value + ['1'])


class Leaf:
    def __init__(self, char):
        self.char = char

    def walk(self, bin_, value):
        bin_[self.char] = value


class string_func:
    def __init__(self, string):
        self.string = string
        self.bin_ = {}
        self.node = self.get_node()
        self.node.walk(self.bin_, [])

    def get_leafes(self, string):
        new_counted = {}
        for key, val in dict(Counter(string)).items():
            new_counted[Leaf(key)] = val
        return list(new_counted.items())

    def get_node(self):
        items = self.get_leafes(self.string)
        while len(items) >= 2:
            left_leaf = items.pop()
            right_leaf = items.pop()
            leaf_count = left_leaf[1] + right_leaf[1]
            items.append((MyNode(left=left_leaf[0], right=right_leaf[0]), leaf_count))
            items.sort(key=itemgetter(1), reverse=True)
        return items.pop()[0]

    def __str__(self):
        str_ = []
        for i in self.string:
            str_ += self.bin_[i] + [' ']
        return ''.join(str_)


if __name__ == '__main__':
    x = string_func(input('Введите любую строку: '))
    print(x)